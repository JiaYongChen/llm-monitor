...
export default function Sidebar() {
  ...
  return (
    <aside className="flex flex-col h-screen select-none" style={{ width: 230, background: '#fafafc', borderRight: '1px solid #e5e5ea' }}>
      <div className="px-4 py-3.5 flex items-center gap-2.5" style={{ borderBottom: '1px solid #e5e5ea' }}>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-3">
        ...
        <div className="mb-4">
          ...
          {Object.entries(META).map(([tool, m]) => (
            <button
              key={tool}
              onClick={() => setParam('tool', selectedTool === tool ? '' : tool)}
        ...
        <div className="mb-4">
          ...
          {enabledProviders.map((p: any) => (
            <button
              key={p.provider}
              onClick={() => setParam('provider', selectedProvider === p.provider ? '' : p.provider)}
      ...
      <div className="px-3 py-2.5" style={{ borderTop: '1px solid #e5e5ea' }}>
        <Link to="/settings" className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors ${loc.pathname === '/settings' ? 'bg-[#e8e7ff] text-[#5e5ce6]' : 'text-[#6e6e73] hover:bg-[#f0f0f4]'}`}>
