  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5 animate-in">
      ...
      <div className="rounded-xl bg-white border border-[#e5e5ea] shadow-sm p-5">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <span className="px-2 py-0.5 rounded-md text-xs font-mono font-medium bg-[#f0f0f4] text-[#6e6e73]">{call.method}</span>
            <span className="font-mono text-sm text-[#1d1d1f]">{call.endpoint}</span>
          </div>
          <span className="px-2.5 py-1 rounded-md text-xs font-mono font-semibold" style={{ background: ok ? '#e6f7f1' : '#fff0f0', color: sc }}>
            {call.status_code}{ok ? ' OK' : call.error_message ? ` ${call.error_message}` : ''}
          </span>
        </div>
        <div className="flex gap-5 text-xs font-mono text-[#aeaeb2]">
          <span>{call.provider}</span><span title={call.model}>{call.model}</span>
          <span>{(call.duration_ms / 1000).toFixed(2)}s</span><span>{call.created_at}</span>
        </div>
      </div>
      ...
      {call.prompt_tokens != null && (
        <div className="rounded-xl bg-white border border-[#e5e5ea] shadow-sm p-5">
          <h3 className="text-sm font-semibold text-[#1d1d1f] mb-4">费用明细</h3>
      ...
      <CodeBlock title="请求" raw={call.request_body} />
      <CodeBlock title="响应" raw={call.response_body} />
    </div>
  );
}
