...
import {
  listProviderModels, replaceProviderModels, seedProviderModels, setModelEnabled, deleteProviderModels,
...
describe('provider_models', () => {
  it('新增模型行（available=1 enabled=1 含时间戳）', () => {
    const now = Date.now();
    replaceProviderModels('openai', ['gpt-5.6-sol', 'gpt-5.6-luna'], null, now);
  ...
  it('探测不到的存量模型置灰不删除，用户 enabled 状态不重置', () => {
    ...
    replaceProviderModels('openai', ['gpt-5.6-sol'], null, now + 1000); // luna 不再探测到
  ...
  it('重新探测到置灰模型后 available 恢复 1（enabled 保持关闭）', () => {
    replaceProviderModels('openai', ['gpt-5.6-sol', 'gpt-5.6-luna'], null, Date.now());
  ...
  it('删除供应商联动删除模型行', () => {
    ...
    replaceProviderModels('custom-prov', ['m1'], null, Date.now());
  ...
  it('seedProviderModels 行缺失才插入，已有行不动', () => {
    const items = [{ provider: 'seed-prov', model: 'm1', input_price: 1, cache_input_price: 0.5, output_price: 2, currency: 'USD' }];
    expect(seedProviderModels(items, Date.now())).toBe(1);
    // 已有行不覆盖
    expect(seedProviderModels([{ ...items[0], input_price: 999 }], Date.now())).toBe(0);
    const row = listProviderModels().find(r => r.provider === 'seed-prov' && r.model === 'm1')!;
